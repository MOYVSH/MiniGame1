    ...
    public class AnimationUIInspector : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (!animationUI.IsPlayingInEditMode)
            {
                ...
                {
                    InvokeNonPublicMethod("PreviewAnimation"); //animationUI.PreviewAnimation();
                }
            }
            else
            {
            ...
            if (GUILayout.Button("Preview Start"))
            {
                ...
                InvokeNonPublicMethod("PreviewStart");
            }
            else if (GUILayout.Button("Preview End"))
            {
                // animationUI.PreviewEnd();
                InvokeNonPublicMethod( "PreviewEnd");
            }
            ...
            if (Application.isPlaying)
            {
                ...
                {
                    animationUI.PlayForward();
                }
                else if (GUILayout.Button("PlayBackwards"))
                {
                    animationUI.PlayBackwards();
                }
            ...
            InvokeNonPublicMethod("InitTime");
            ...
            if (sliderValue != animationUI.CurrentTime) //Happens when dragging progess bar
            {
                ...
                if (!animationUI.IsPlayingInEditMode)
                    ...
                    InvokeNonPublicMethod("UpdateBySlider");
            }
            ...
            animationUI.Loop = GUILayout.Toggle(animationUI.Loop, new GUIContent("Loop"));
            ...
            foreach (Sequence sequence in animationUI.AnimationSequence)
            {
                ...
                {
                    float currentSideLoadingRectHeight =
                        (sequence.Duration == 0) ? currentRect.height : Mathf.Lerp(0, currentRect.height, (animationUI.CurrentTime - sequence.StartTime) / sequence.Duration);
        ...
        object InvokeNonPublicMethod(string method, params object[] parameters)
        {
            System.Type type = typeof(AnimationUI);
            BindingFlags flag = BindingFlags.Instance | BindingFlags.InvokeMethod | BindingFlags.NonPublic | BindingFlags.Public;
            var methodInfo = type.GetMethod(method, flag);
            if (methodInfo == null)
            {
