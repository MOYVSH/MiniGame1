...
namespace QFramework
{
    ...
    {
        public static IArchitecture CurrentArchitecture { set; get; }
    }
    
    public interface IArchitecture
    {
        void RegisterProxy<T>() where T : class, IArchitectureProxy, new();
        ...
        T GetSystem<T>(bool autoCreate = true) where T : class, ISystem;
    ...
    {
        private Status mInitStatus = Status.NotDo;
        
        public static Action<T> OnRegisterPatch = architecture => { };
        
        protected static T mArchitecture;
        
        private readonly Dictionary<Type, IArchitectureProxy> mProxies = new Dictionary<Type, IArchitectureProxy>();
        ...
        internal static T Instance => mArchitecture;
        
        private static void MakeSureArchitecture()
        {
            if (mArchitecture.mInitStatus == Status.NotDo)
            {
                mArchitecture.mInitStatus = Status.Doing;
                mArchitecture.Init();
                OnRegisterPatch?.Invoke(mArchitecture);
                mArchitecture.mInitStatus = Status.Did;
            }
            
#if ENABLE_MULTITHREAD_EVENT
            if (mArchitecture != null)
            {
                var timer = mArchitecture.GetSystem<TimerSystem>();
                if (timer != null && lastFrameIndex != Time.frameCount)
                {
                    lastFrameIndex = Time.frameCount;
                    timer.AddFrameTask(mArchitecture.HandleMainThreadEvents, true);
                }
            }
#endif
        }
        ...
        {
            OnReinit();
            var models = GetAllModels().Where(m => m.InitStatus == Status.NotDo);
            foreach (var model in models) model.Init();
            var systems = GetAllSystems().Where(s => s.InitStatus == Status.NotDo);
        ...
        public void Reset()
        {
            OnReset();
            var systems = GetAllSystems().Where(s => s.InitStatus != Status.NotDo);
            foreach (var system in systems) system.Reset();
            var models = GetAllModels().Where(m => m.InitStatus != Status.NotDo);
        ...
        public void ClearAll()
        {
            ...
            ClearEvent();
            ClearContainer();
            mArchitecture.mInitStatus = Status.NotDo;
            mArchitecture.mProxies.Clear();
        }
        
        internal IOCContainer mContainer = new IOCContainer();
        
        public void HandleMainThreadEvents()
        {
            mTypeEventSystem.TriggerEventOnMainThread();
        }
        
        public void RegisterProxy<TProxy>() where TProxy : class, IArchitectureProxy, new()
        {
            var type = typeof(TProxy);
            if (!mProxies.ContainsKey(type))
            {
                var proxy = mProxies[type] = new TProxy();
                proxy.Init();
            }
        }
        ...
        public void RegisterSystem<TSystem>(TSystem system) where TSystem : class, ISystem
        {
            ...
            var result = mContainer.Register(system);
            if (result.InitStatus == Status.NotDo && !result.LazyInit) result.Init();
        }
        
        public void RegisterModel<TModel>(TModel model) where TModel : class, IModel
        {
            model.SetArchitecture(this);
            var result = mContainer.Register(model);
            if (result.InitStatus == Status.NotDo && !result.LazyInit) result.Init();
        }
        ...
        public TSystem GetSystem<TSystem>(bool autoCreate = true) where TSystem : class, ISystem
        {
            var system = mContainer.Get<TSystem>(false);
            if (system != null)
            {
                if (system.InitStatus == Status.NotDo && system.LazyInit) system.Init();
                return system;
            }
            
            if (autoCreate)
            {
                system = mContainer.Register(Activator.CreateInstance<TSystem>());
                system.SetArchitecture(this);
                if (system.InitStatus == Status.NotDo) system.Init();
            }
        ...
        {
            var result = mContainer.Get(type);
            if (result is ISystem system)
            {
                if (system.InitStatus == Status.NotDo && system.LazyInit) system.Init();
                return system;
            }
            
            system = (ISystem) mContainer.Register(Activator.CreateInstance(type));
            system.SetArchitecture(this);
            if (system.InitStatus == Status.NotDo) system.Init();
            return system;
        }
        ...
        {
            List<ISystem> results = new List<ISystem>();
            foreach (var pair in mContainer.GetContainer().Entry)
            {
                if (pair.Value is ISystem system)
                {
                    results.Add(system);
                }
            }
        ...
        {
            var model = mContainer.Get<TModel>();
            if (model != null && model.InitStatus == Status.NotDo && model.LazyInit) model.Init();
            return model;
        }
        ...
        public IModel GetModel(Type type)
        {
            ...
            {
                if (model.InitStatus == Status.NotDo && model.LazyInit) model.Init();
                return model;
            }
        ...
        public void SendCommand<TCommand>() where TCommand : ICommand, new() => ExecuteCommand(new TCommand());
        ...
        public void SendEventToMainThread<TEvent>() where TEvent : new() => mTypeEventSystem.SendToMainThread<TEvent>();
        
        public void SendEventToMainThread<TEvent>(TEvent e) => mTypeEventSystem.SendToMainThread<TEvent>(e);
        ...
        public void ClearEvent() => mTypeEventSystem.Clear();
        
        public void ClearContainer() => mContainer.Clear();
    }
    ...
    public abstract class AbstractMonoBehaviourController : MonoBehaviour, IController
    {
        public abstract IArchitecture GetArchitecture();
        
        public virtual string logTag => this.GetType().Name;
        
        public virtual void Log(object msg, GameObject go = null)
        {
            Debug.Log($"[{logTag}] {msg}", go);
        }
        
        public virtual void LogWarning(object msg, GameObject go = null)
        {
            Debug.LogWarning($"[{logTag}] {msg}", go);
        }
        
        public virtual void LogError(object msg, GameObject go = null)
        {
            Debug.LogError($"[{logTag}] {msg}", go);
        }
    }
    ...
    public abstract class AbstractSystem : AbstractLogger, ISystem
    {
        ...
        public Status InitStatus { get; protected set; }
        
        public virtual bool LazyInit => false;
        
        public void Init()
        {
            InitStatus = Status.Doing;
            OnInit();
            InitStatus = Status.Did;
        }
    ...
    public abstract class AbstractModel : AbstractLogger, IModel
    {
        private IArchitecture mArchitecture;
    ...
    public abstract class AbstractCommand<TResult> : AbstractLogger, ICommand<TResult>
    {
        private IArchitecture mArchitecture;
    ...
    public interface IBelongToArchitecture
    {
        ...
        string logTag { get; }
        void Log(object msg, GameObject context = null);
        void LogWarning(object msg, GameObject context = null);
        void LogError(object msg, GameObject context = null);
    }
    ...
    public static class CanGetModelExtension
    {
        ...
        public static IModel GetModel(this ICanGetModel self, Type type) => self.GetArchitecture().GetModel(type);
    }
    ...
    {
        public static T GetSystem<T>(this ICanGetSystem self, bool autoCreate = true) where T : class, ISystem =>
            self.GetArchitecture().GetSystem<T>(autoCreate);
        
        public static ISystem GetSystem(this ICanGetSystem self, Type type) => self.GetArchitecture().GetSystem(type);
    }
    ...
    public static class CanSendEventExtension
    {
        ...
        public static void SendEventToMainThread<T>(this ICanSendEvent self) where T : new() =>
            self.GetArchitecture().SendEventToMainThread<T>();
        
        public static void SendEventToMainThread<T>(this ICanSendEvent self, T e) =>
            self.GetArchitecture().SendEventToMainThread<T>(e);
    }
    ...
    public class TypeEventSystem
    {
        ...
        private readonly ConcurrentQueue<Action> mMainThreadEvents = new ConcurrentQueue<Action>(new List<Action>(64));
        ...
        public void SendToMainThread<T>() where T : new() =>
            mMainThreadEvents.Enqueue(() => mEvents.GetEvent<EasyEvent<T>>()?.Trigger(new T()));
        
        public void SendToMainThread<T>(T e) =>
            mMainThreadEvents.Enqueue(() => mEvents.GetEvent<EasyEvent<T>>()?.Trigger(e));
        ...
        public void TriggerEventOnMainThread()
        {
            if (mMainThreadEvents.Count > 0 && mMainThreadEvents.TryDequeue(out var action))
            {
                action?.Invoke();
            }
        }
    ...
    public abstract class AbstractLogger : IBelongToArchitecture
    {
        public abstract IArchitecture GetArchitecture();
    ...
    {
        private MultipleKeysDictionary<Type, object> mInstances = new MultipleKeysDictionary<Type, object>();
        
        public MultipleKeysDictionary<Type, object> GetContainer()
        {
            return mInstances;
        }
        ...
        public T Register<T>(T instance) where T : class
        {
            ...
            if (mInstances.TryGetValue(key, out var old) || mInstances.TryGetValue(instanceType, out old))
            {
                if (old != null)
                {
                    FillToDict(old, types);
                    Debug.Log($"Already registered type:{key.FullName} with {instanceType.FullName}");
                    return old as T;
                }
            }
            
            FillToDict(instance, types);
            return instance;
        }
        
        private void FillToDict<T>(T instance, Type[] types)
        {
            foreach (Type type in types)
            {
                if (type != null && !type.FullName.StartsWith("QFramework."))
                {
                    mInstances[type] = instance;
                }
            }
            
            mInstances[instance.GetType()] = instance;
        }
        
        public T Get<T>(bool logError = true) where T : class
        {
            var key = typeof(T);
            ...
            if (logError)
                Debug.LogError($"Get {typeof(T).FullName} from container failed");
            return null;
        }
        
        public object Get(Type type)
        {
            if (mInstances.TryGetValue(type, out var retInstance))
            {
                return retInstance;
            }
        ...
        {
            var type = typeof(T);
            return mInstances.Entry.Values.Where(instance => type.IsInstanceOfType(instance)).Cast<T>();
        }
